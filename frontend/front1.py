...
if df is not None:
    ...
    with col1:
        ...
        if ajuste:
            m,b,sm,sb, cfit = ajuste_lineal(x = df[x].values, y = df[y].values, sigmay = df[erry].values if erry else None)
    ...
    usar_ejes = st.sidebar.checkbox("Ejes sin etiqueta")
    ...
    if not usar_ejes:
        ...
        with col1:
            pers_xlabel = st.checkbox('Personalizar etiqueta en x')
            pers_ylabel = st.checkbox('Personalizar etiqueta y')
